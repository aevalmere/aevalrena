...
- 2026-09-10 balance: full-charge fsmash KOs at 64 percent against the 90 percent target stated in docs/SPEC.md:572. Logged, not fixed. Measured 64 both before and after the balance JSON handoff, so the gap is pre-existing and unrelated to that change. fsmash sits at 19/46 in moves.ts against 40/100 in the original spec table, so roughly halving bkb and kbg still overshoots the target, which points at the knockback formula in the sim rather than the move's own numbers. Deferred until after the current art wave.
- 2026-09-10 balance: balance/aeval.balance.json is read by nothing in src/, so it is documentation only and moves.ts stays hand-maintained and authoritative. Its global_character_tuning multipliers (projectile_knockback 0.95, grounded_melee_damage 0.95, landing_lag_bias 1.0, recovery_safety_bias 0.95) are an unbuilt runtime layer and are not baked into the per-move numbers, which are whole integers identical to the pre-handoff code. The JSON and moves.ts will drift apart again without a loader or a CI check that diffs the two.
